...
mod tests {
    ...
    impl Perform for Dispatcher {
        ...
        fn apc_abort(&mut self) {
            self.dispatched.push(Sequence::ApcAbort);
        }
    }

    #[test]
    fn parse_apc_across_every_split() {
        const INPUT: &[u8] = b"\x1b_Gf=32,s=1,v=1;AAAA\x1b\\";
        let expected = [
            Sequence::ApcStart,
            Sequence::ApcPut(b'G'),
            Sequence::ApcPut(b'f'),
    ...
    fn cancel_apc() {
        for cancel in [0x18, 0x1A] {
            let mut dispatcher = Dispatcher::default();
            let mut parser = Parser::new();
            parser.advance(&mut dispatcher, &[b'\x1b', b'_', b'G', b'a', cancel]);
            ...
            assert_eq!(dispatcher.dispatched, [
                ...
                Sequence::ApcPut(b'a'),
                Sequence::ApcAbort,
                Sequence::Execute(cancel),
            ]);
        }
    }

    #[test]
    fn sos_and_pm_remain_ignored() {
        let mut dispatcher = Dispatcher::default();
        let mut parser = Parser::new();
        parser.advance(&mut dispatcher, b"\x1bXignored\x1b\\\x1b^ignored\x1b\\");
